...
# données Arches
identifiants_features <- read.csv(file = "EAMENA_2021-09-06_13-29-56.csv") %>%
  as_tibble()

identifiants_photos_sketches <- read.csv(file = "EAMENA_2021-09-06_14-12-28.csv") %>%
  as_tibble()
  

# données initiales de relations photos & features
relations_photos <- read_excel(path = "UCOP_relations_photos_features_places_bulk_1.xlsx")
