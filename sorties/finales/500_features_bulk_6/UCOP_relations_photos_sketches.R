...
identifiants_photos_sketches <- read.csv(file = "EAMENA_2021-09-23_05-01-56.csv") %>%
  as_tibble() %>%
  bind_rows(read.csv(file = "EAMENA_2021-09-23_05-06-19.csv") %>%
              as_tibble()) %>%
  bind_rows(read.csv(file = "EAMENA_2021-09-23_05-09-45.csv") %>%
              as_tibble()) %>%
  bind_rows(read.csv(file = "EAMENA_2021-09-23_05-11-35.csv") %>%
              as_tibble()) %>%
  unique()
  

# données initiales de relations photos & features
relations_photos <- read_excel(path = "UCOP_relations_photos_features_places_bulk_number_6.xlsx")
...
openxlsx::write.xlsx(relations_photos_liste,
                     "UCOP_relations_photos_features_places_bulk_6_Arches_ID.xlsx",
                     append = TRUE)
