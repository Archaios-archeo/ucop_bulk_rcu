...
identifiants_photos_sketches <- read.csv(file = "EAMENA_2021-09-23_04-49-19.csv") %>%
  as_tibble() %>%
  bind_rows(read.csv(file = "EAMENA_2021-09-23_04-51-58.csv") %>%
              as_tibble())
  

# données initiales de relations photos & features
relations_photos <- read_excel(path = "UCOP_relations_photos_features_places_bulk_number_5.xlsx")
...
openxlsx::write.xlsx(relations_photos_liste,
                     "UCOP_relations_photos_features_places_bulk_5_Arches_ID.xlsx",
                     append = TRUE)
